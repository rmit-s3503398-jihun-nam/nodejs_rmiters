...
var storage = multer.diskStorage({
  ...
  {
     callback(null,'./public/uploads/buyandsell');
  },
 ...
 */
  ...
  */
    ...
    if(refinedCampues.length==0)
    {
 	...
 	var buyandsellListLimit = req.body.limit || config.buyandsellListLimit;
 	var skip = req.body.skip || 0;

 	 BuyModel.find(offerTypeData,function(err,lists){
 	 ...
 	 }).sort({"post_date":"desc"}).skip(skip).limit(buyandsellListLimit).or(refinedCampues);
 }
 ...
 *   filter array pushes orginal image name and check if same image already pushed in fileswillupload
 */
 ...
 exports.postNewAd = function(req,res,next)
 {
			...
			if(req.body.filesWillUpload.indexOf(req.files[i].originalname)!=-1 && filter.indexOf(req.files[i].originalname)==-1)
			{
				...
				filesWillUpload.push(req.files[i].filename);
 				filter.push(req.files[i].originalname);
					...
					easyimg.resize({
					     src:'./public/uploads/buyandsell/'+ req.files[i].filename, 
					     dst:'./public/uploads/buyandsell/thumbs/'+ req.files[i].filename,
	...
	*/ 
	...
	if(mainImage=="" || (filesWillUpload.length>0 && filesWillUpload.indexOf(mainImage)==-1))
	{
	    ...
	    {
	    	mainImage = config.buyandsellDefaultImage;
	    }	
	}
